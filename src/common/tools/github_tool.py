...
class GitHubTools:
    def __init__(self, auth_method: AuthMethod = AuthMethod.TOKEN):
        ...
        if auth_method == AuthMethod.TOKEN:
            ...
            if not self.app_id or not self.private_key_path:
                logger.error("GITHUB_APP_ID or GITHUB_PRIVATE_KEY_PATH environment variable not set")

            if not self.installation_id:
                logger.error("GITHUB_INSTALLATION_ID environment variable not set")
    ...
    def _get_jwt_token(self):
        ...
        try:
            ...
            logger.info(f"Using private key at: {abs_key_path}")
            ...
            #logger.info(f"[cyan]Creating JWT with app_id: {self.app_id}[/cyan]")
            ...
            logger.info("[green]JWT token generated successfully[/green]")
            return token
        except Exception as e:
            logger.info(f"[red]Error generating JWT token: {str(e)}[/red]")
            return None
    ...
    def _refresh_installation_token(self):
        ...
        """
        if not self.installation_id:
            logger.info("[red]Error: No installation ID available[/red]")
            return
        ...
        if self.installation_token:
            ...
            logger.info("[green]Installation token refreshed successfully[/green]")
        else:
            logger.info("[red]Failed to refresh installation token[/red]")
    ...
    def _get_installation_token(self, installation_id: int):
        ...
        try:
            ...
            if not self.token or (self.jwt_token_expires_at and datetime.now() >= self.jwt_token_expires_at):
                logger.info("[yellow]JWT token expired or missing, refreshing...[/yellow]")
                self._refresh_jwt_token()

            if not self.token:
                logger.info("[red]Error: JWT token is required to get an installation token[/red]")
                return None
            ...
            logger.info(f"[cyan]Requesting installation token for installation ID: {installation_id}[/cyan]")
            response = requests.post(url, headers=headers)

            if response.status_code != 201:
                logger.info(f"[red]Error getting installation token: {response.status_code} - {response.text}[/red]")
                return None
            ...
            logger.info(f"[red]Error generating installation token: {str(e)}[/red]")
            return None
    ...
    def _get_auth_headers(self):
        ...
        if self.auth_method == AuthMethod.APP:
            ...
            if self.installation_token_expires_at and datetime.now() >= self.installation_token_expires_at:
                logger.info("[yellow]Installation token expired, refreshing...[/yellow]")
                self._refresh_installation_token()
            ...
            if self.jwt_token_expires_at and datetime.now() >= self.jwt_token_expires_at:
                logger.info("[yellow]JWT token expired, refreshing...[/yellow]")
                self._refresh_jwt_token()

            # Fall back to JWT token if no installation token
            if not self.token:
                logger.info("[red]Error: JWT token for GitHub App is None[/red]")
    ...
    def fetch_github_issue(self, owner: str, repo: str, issue_number: int):
        ...
        """
        logger.info("[cyan]USE TOOL FETCH_ISSUE[/cyan]")
    ...
    def fetch_github_directory(self, owner: str, repo: str, path: str, branch: str = "main"):
        ...
        """
        logger.info("[cyan]USE TOOL FETCH_DIRECTORY[/cyan]")
        url = f"https://api.github.com/repos/{owner}/{repo}/contents/{path}?ref={branch}"
        logger.info(f"[cyan]Fetching from {url}[/cyan]")
        ...
        if not isinstance(contents, list):
            logger.info("[cyan]Processing single file[/cyan]")
    ...
    def create_github_branch(self, owner: str, repo: str, new_branch: str):
        ...
        logger.info("[cyan]USE TOOL CREATE_BRANCH[/cyan]")
    ...
    def update_github_file(
        ...
        Returns:
            dict: API response or error message.
        """
        logger.info(f"[cyan]USE TOOL UPDATE_FILE on branch {branch}[/cyan]")
    ...
    def fetch_github_pr_changes(self, owner: str, repo: str, pr_number: int):
        ...
        logger.info(f"[cyan]USE TOOL FETCH_PR_CHANGES for PR #{pr_number}[/cyan]")
    ...
    def post_github_comment(
        ...
        logger.info(f"[cyan]USE TOOL POST_COMMENT[/cyan]")
